...
public class ChiliFragment extends Fragment implements View.OnClickListener {
    ...
    public ChiliFragment() {
    }

    @Override
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
                             Bundle savedInstanceState) {
        return inflater.inflate(R.layout.fragment_cabai, container, false);
    }

    @Override
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        super.onViewCreated(view, savedInstanceState);
        pbCircle = view.findViewById(R.id.pb_circle_chili);
        tvTime = view.findViewById(R.id.tv_time_chili);
        tvDateEnd = view.findViewById(R.id.tv_date_end_chili);

        btnStart = view.findViewById(R.id.btn_start_chili);
        btnHistory = view.findViewById(R.id.btn_history_chili);
        ...
        refresh = view.findViewById(R.id.refresh_chili);
    ...
    public void onStart() {
        super.onStart();
        if (pref.getStatus(TimerPreference.CHILI)) {
            checkPlant();
        }
    }
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.btn_start_chili:
                helper.postStatus(TimerPreference.CHILI, "on");
                pref.setStatus(TimerPreference.CHILI, true);
                pref.setInterval(TimerPreference.CHILI_INTERVAL, 600);
                tvTime.clearAnimation();
                tvTime.animate().cancel();
                startTimer();
                btnStart.setEnabled(false);
                break;
            case R.id.btn_history_chili:
                showDialog();
                break;
        }
    }
    ...
    private void startTimer() {
        isRunning = true;
        btnStart.setEnabled(false);
        final int second = pref.getInterval(TimerPreference.CHILI_INTERVAL);
        timer = new CountDownTimer(second * 1000, 500) {
            ...
            public void onFinish() {
                ...
                pref.setStatus(TimerPreference.CHILI, false);
                pref.setInterval(TimerPreference.CHILI_INTERVAL, 0);
                ...
                helper.showNotification(
                        ...
                        String.format(getResources().getString(R.string.panen_desc), "Cabai"),
                        1
                );
                blink(500);
                helper.postStatus(TimerPreference.CHILI, "off");
                isRunning = false;
            }
        }.start();
    }
    ...
    private void checkPlant() {
        helper.loadPlant(TimerPreference.CHILI, TimerPreference.CHILI_INTERVAL);
        if (pref.getStatus(TimerPreference.CHILI)) {
            if (isRunning) {
                timer.cancel();
            }
    ...
    private void showDialog() {
        ...
        bundle.putString("TAG",TimerPreference.CHILI);
