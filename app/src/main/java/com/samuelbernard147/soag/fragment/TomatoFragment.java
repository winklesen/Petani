...
public class TomatoFragment extends Fragment implements View.OnClickListener {
    ...
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        ...
        pbCircle = view.findViewById(R.id.pb_circle_tomato);
        tvTime = view.findViewById(R.id.tv_time_tomato);
        tvDateEnd = view.findViewById(R.id.tv_date_end_tomato);

        btnStart = view.findViewById(R.id.btn_start_tomato);
        btnHistory = view.findViewById(R.id.btn_history_tomato);
        ...
        refresh = view.findViewById(R.id.refresh_tomato);
    ...
    public void onStart() {
        super.onStart();
        if (pref.getStatus(TimerPreference.TOMATO)) {
            checkPlant();
        }
    }
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.btn_start_tomato:
                helper.postStatus(TimerPreference.TOMATO, "on");
                pref.setStatus(TimerPreference.TOMATO, true);
                pref.setInterval(TimerPreference.TOMATO_INTERVAL, 600);
                tvTime.clearAnimation();
                tvTime.animate().cancel();
                startTimer();
                btnStart.setEnabled(false);
                break;
            case R.id.btn_history_tomato:
                showDialog();
                break;
        }
    }
    ...
    private void startTimer() {
        isRunning = true;
        btnStart.setEnabled(false);
        final int second = pref.getInterval(TimerPreference.TOMATO_INTERVAL);
        timer = new CountDownTimer(second * 1000, 500) {
            ...
            public void onFinish() {
                ...
                pref.setStatus(TimerPreference.TOMATO, false);
                pref.setInterval(TimerPreference.TOMATO_INTERVAL, 0);
                ...
                helper.showNotification(
                        ...
                        String.format(getResources().getString(R.string.panen_desc), "Tomat"),
                        4
                );
                blink(500);
                helper.postStatus(TimerPreference.TOMATO_INTERVAL, "off");
                isRunning = false;
            }
        }.start();
    }
    ...
    private void checkPlant() {
        helper.loadPlant(TimerPreference.TOMATO, TimerPreference.TOMATO_INTERVAL);
        if (pref.getStatus(TimerPreference.TOMATO)) {
            if (isRunning) {
                timer.cancel();
            }
    ...
    private void showDialog() {
        ...
        bundle.putString("TAG",TimerPreference.TOMATO);
