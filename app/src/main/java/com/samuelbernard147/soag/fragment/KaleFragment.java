...
public class KaleFragment extends Fragment implements View.OnClickListener{
    ...
    public View onCreateView(LayoutInflater inflater, ViewGroup container,
        ...
        return inflater.inflate(R.layout.fragment_kangkung, container, false);
    }

    @Override
    public void onViewCreated(@NonNull View view, @Nullable Bundle savedInstanceState) {
        super.onViewCreated(view, savedInstanceState);
        pbCircle = view.findViewById(R.id.pb_circle_kale);
        tvTime = view.findViewById(R.id.tv_time_kale);
        tvDateEnd = view.findViewById(R.id.tv_date_end_kale);

        btnStart = view.findViewById(R.id.btn_start_kale);
        btnHistory = view.findViewById(R.id.btn_history_kale);
        ...
        refresh = view.findViewById(R.id.refresh_kale);
    ...
    public void onStart() {
        super.onStart();
        if (pref.getStatus(TimerPreference.KALE)) {
            checkPlant();
        }
    }
    ...
    public void onClick(View view) {
        switch (view.getId()) {
            case R.id.btn_start_kale:
                helper.postStatus(TimerPreference.KALE, "on");
                pref.setStatus(TimerPreference.KALE, true);
                pref.setInterval(TimerPreference.KALE_INTERVAL, 600);
                tvTime.clearAnimation();
                tvTime.animate().cancel();
                startTimer();
                btnStart.setEnabled(false);
                break;
            case R.id.btn_history_kale:
                showDialog();
                break;
        }
    }
    ...
    private void startTimer() {
        isRunning = true;
        btnStart.setEnabled(false);
        final int second = pref.getInterval(TimerPreference.KALE_INTERVAL);
        timer = new CountDownTimer(second * 1000, 500) {
            ...
            public void onFinish() {
                ...
                pref.setStatus(TimerPreference.KALE, false);
                pref.setInterval(TimerPreference.KALE_INTERVAL, 0);
                ...
                helper.showNotification(
                        ...
                        String.format(getResources().getString(R.string.panen_desc), "Kangkung"),
                        2
                );
                blink(500);
                helper.postStatus(TimerPreference.KALE, "off");
                isRunning = false;
            }
        }.start();
    }
    ...
    private void checkPlant() {
        helper.loadPlant(TimerPreference.KALE, TimerPreference.KALE_INTERVAL);
        if (pref.getStatus(TimerPreference.KALE)) {
            if (isRunning) {
                timer.cancel();
            }
    ...
    private void showDialog() {
        ...
        bundle.putString("TAG",TimerPreference.KALE);
